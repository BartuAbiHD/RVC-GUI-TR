...
def vc_single(
    ...
    try:
        ...
        return "Başarılı", (tgt_sr, audio_opt)
    except:
...
def browse_zip():
    ...
    zip_file = filedialog.askopenfilename(
        ...
        title="Dosya seç",
        filetypes=(("Zip Dosyası", "*.zip"), ("Tüm Dosyalar", "*.*")),
    )
...
def browse_file():
    filepath = filedialog.askopenfilename (
        filetypes=[("Ses Dosyaları", "*.wav;*.mp3")])
...
root.title("RVC GUI TURKISH [Trias AI] - [bartucivas.com.tr]")
...
sid_label = ctk.CTkLabel(select_model_frame, text="Konuşmacı Kimliği:")
...
select_model = ctk.StringVar(value="Bir model seçin")
...
input_audio_label = ctk.CTkLabel(inputpath_frame, text="Giriş ses dosyası:")
browse_button = ctk.CTkButton(
    inputpath_frame, text="Gözat", command=browse_file)
...
file_index_label = ctk.CTkLabel(right_frame, text=".index Dosyası (Önerilir)")
...
index_rate_label = ctk.CTkLabel(
    right_frame, text="Özellik alma oranı: 0.4" )
...
notes_label = ctk.CTkLabel(left_frame, justify="left", text_color="#8A8A8A", text="İpuçları: \n 1. harvest ve crepe en kaliteli, aynı zamanda en yavaş yöntemlerdir. \n\n 2. dio ve pm en hafif ve en hızlı yöntemlerdir, aynı zamanda en düşük kalitedir. \n\n 3. Eğer kadın sesini erkek ses modeline söyletecekseniz Pitch değerini -12 olarak, \n eğer erkek sesini kadın ses modeline söyletecekseniz 12 olarak ayarlayın. \n Söyleteceğiniz ses ile model aynı cinsiyet ise de Pitch değerini 0 olarak bırakın.")

# intiilizing loading progress bar widget

loading_frame = ctk.CTkFrame(master=root, width=200) 

laoding_label = ctk.CTkLabel(loading_frame, text="Dönüştürülüyor..., Pencere yanıt vermiyorsa, lütfen bekleyin.")
...
change_device_label = ctk.CTkLabel( right_frame, text="İşleme modu")
...
last_output_label = ctk.CTkLabel(output_audio_frame, text="Çıkış yolu: ")
last_output_file = ctk.CTkLabel(output_audio_frame, text="", text_color="green")
open_output_button = ctk.CTkButton(output_audio_frame, text="Aç", command=lambda: play_audio(output_file))

# intiilizing import models button widget
import_moodels_button = ctk.CTkButton(right_frame, fg_color="darkred", hover_color="black", corner_radius=20, text=".zip'den modeli içe aktar", command=browse_zip)
